...
export default function SubscribeForm({ inputBg = 'bg-surface' }: SubscribeFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success');
      setMessage(data.message === 'Already subscribed' ? 'You\'re already on the list!' : 'You\'re in! We\'ll notify you at launch.');
      setEmail('');
    } catch {
      setStatus('error');
    ...
    <div>
      <form className="flex gap-2" onSubmit={handleSubmit}>
        <input
          ...
          required
          className={`flex-1 px-4 py-3 rounded-full border border-border ${inputBg} text-text text-sm focus:outline-none focus:ring-2 focus:ring-primary`}
        />
        <button
          type="submit"
